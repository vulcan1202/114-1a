...
erDiagram
    %% 定義主表 AudioTask
    AudioTask {
        UUID id PK "主鍵 (Primary Key)"
        string celery_task_id "Celery 任務 ID"
        string file_name "原始檔名"
        string audio_file "檔案路徑"
        string status "任務狀態 (PENDING/PROCESSING...)"
    ...
    %% 定義關聯表 TranscriptSegment
    TranscriptSegment {
        int id PK "主鍵"
        UUID task_id FK "外鍵 (連回 AudioTask)"
        string speaker "說話者"
        float start_time "開始時間 (秒)"
        float end_time "結束時間 (秒)"
        text text "分段文字內容"
    }

    %% 定義關係：一對多 (One-to-Many)
    %% 一個 AudioTask 包含多個 TranscriptSegment
    AudioTask ||--o{ TranscriptSegment : "包含 (1:N)"
```
